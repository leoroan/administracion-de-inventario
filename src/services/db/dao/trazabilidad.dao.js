    ...
    try {
      ...
      if (equipo) {
        ...
        if (userId) {
          ...
          result = await this.model.create({ nombreEmpleado: nombreEmpleado, dniEmpleado: dniEmpleado, mtEquipo: mtEquipo, idEquipo: equipId, nombreOficina: nombreOficina, registroDeMantenimientoDeEquipoId: registroId, estado: tipoMovimiento, equipoId: equipoId, originario: originario });
        } else {
          const oficina = await oficinaService.findById(oficinaId);
          const nombreOficina = oficina.dataValues.nombre;
          result = await this.model.create({ nombreEmpleado: "ES OFICINA", dniEmpleado: "NO CORRESPONDE", mtEquipo: mtEquipo, idEquipo: equipId, nombreOficina: nombreOficina, registroDeMantenimientoDeEquipoId: registroId, estado: tipoMovimiento, equipoId: equipoId, originario: originario });
        }
      }
      return result;
    } catch (error) {
